...
export default function CourseDetails() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      setAdding(true);
      await addToCart(course._id, user.token);
      navigate("/cart"); // good UX
    } catch (err) {
      console.error(err);
      alert("Course already in cart or something went wrong");
    } finally {
      setAdding(false);
    }
  };
        ...
        <div className="row g-4">
            ...
            <div className="card shadow-lg rounded overflow-hidden">
              ...
              <div className="card-body p-4">
                ...
                <div className="mb-4">
                  ...
                  {course.modules?.map((module, module_idx) => (
                    <div key={module_idx} className="mb-3 ">
                      <div
                        className="p-3 bg-light border rounded"
                        data-bs-toggle="collapse"
                        data-bs-target={`#module${module_idx}`}
                        aria-expanded="false"
                        aria-controls={`module${module_idx}`}
                        style={{ cursor: "pointer" }}
                      >
                        <span className="fw-medium">{module.title}</span>
                        <div className="collapse" id={`module${module_idx}`}>
                          {module.lessons.map((lesson, lesson_idx) => (
                            <li className="d-flex flex-column">
                              <Link
                                key={lesson_idx}
                                to={`/learn/courses/${course._id}/lessons/${lesson._id}`}
                                state={{ course }}
                                className="text-decoration-none d-flex justify-content-between align-items-center py-2 px-3 border-bottom"
                              >
          ...
          <div className="col-lg-4">
            ...
            >
              ...
              <Link to="/checkout" state ={{courses:[course._id],totalAmount:course.price}}className="btn btn-outline-primary w-100 mb-3">
                Buy Now
              </Link>
              <button onClick={handleAddToCart} className="btn btn-outline-primary w-100" disabled={adding}>
